...
class RRT:
    ...
    def draw_graph(self, rnd=None, path=None):
        ...
        for (ox, oy, size) in self.obstacle_list:   # 画出障碍物，颜色为k（黑色），形状为o（圆），半径为size
            ...
            plt.plot(ox, oy, "ok", ms=30 * size)  # 画一个圆形，颜色为k（黑色），线条粗细为30*size
        ...
        plt.axis([-2, 18, -2, 15]) # 设置x轴-2到18，y轴-2到15
...
def main():
    ...
    path = rrt.rrt_planning(start=[0, 0], goal=[15, 12], animation=show_animation) # 调用rrt方法，获取从起点start到终点goal的路径，show_animation为是否在迭代时画出路线
